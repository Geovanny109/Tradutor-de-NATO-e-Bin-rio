...
export default function TranslatorApp() {
  ...
  const binaryToText = (binary) => {
    try {
      return binary.trim().split(" ").map((bin) => String.fromCharCode(parseInt(bin, 2))).join("");
    } catch { return "Erro ao traduzir binário."; }
  };
  ...
  return (
    ...
    >
      ...
      >
        <h1 className="text-4xl font-bold mb-2 text-center">Tradutor Binário & NATO</h1>
        <p className="text-zinc-400 text-center mb-8">Traduza códigos binários e alfabeto fonético NATO facilmente.</p>
        <div className="grid grid-cols-2 gap-4 mb-6">
          {["binaryToText", "textToBinary", "natoToText", "textToNato"].map((m) => (
            <button
              key={m}
              onClick={() => setMode(m)}
              className={`p-4 rounded-2xl transition-all duration-300 ${mode === m ? "bg-white text-black" : "bg-zinc-800 hover:bg-zinc-700"}`}
            >
              {m === "binaryToText" ? "Binário → Texto" : m === "textToBinary" ? "Texto → Binário" : m === "natoToText" ? "NATO → Texto" : "Texto → NATO"}
            </button>
          ))}
        </div>
        <textarea
        ...
        >
          ...
          <p className="text-zinc-200 break-words whitespace-pre-wrap">{output || "O resultado aparecerá aqui."}</p>
        </motion.div>
      </motion.div>
    </motion.div>
  );
