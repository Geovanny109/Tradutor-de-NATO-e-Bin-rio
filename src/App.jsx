...
export default function TranslatorApp() {
  ...
  const textToNato = (text) => {
    return text.split("").map((char) => {
      ...
      if (reverseNatoMap[upper]) {
        const natoWord = reverseNatoMap[upper];
        return char === char.toLowerCase() ? natoWord.toLowerCase() : natoWord;
      }
      return char;
    }).join(" ");
  };
  ...
  const assemblyToText = (asm) => {
    ...
    } catch { return "Erro: Formato Assembly inválido."; }
  };
  ...
  const subModes = {
    binary: [{ id: "textToBinary", label: "Texto → Binário" }, { id: "binaryToText", label: "Binário → Texto" }],
    nato: [{ id: "textToNato", label: "Texto → NATO" }, { id: "natoToText", label: "NATO → Texto" }],
    assembly: [{ id: "textToAssembly", label: "Texto → Assembly" }, { id: "assemblyToText", label: "Assembly → Texto" }],
  };
    ...
    <div className={`min-h-screen transition-colors duration-500 flex flex-col items-center p-4 md:p-8 ${isDark ? "bg-zinc-950 text-white" : "bg-zinc-50 text-zinc-900"}`}>
      ...
      <main className="w-full max-w-4xl grid gap-8">
        <div className={`flex p-1.5 rounded-2xl ${isDark ? "bg-zinc-900" : "bg-zinc-200"}`}>
          {categories.map((cat) => (
            <button key={cat.id} onClick={() => { setActiveTab(cat.id); setMode(subModes[cat.id][0].id); }} className={`flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all relative ${activeTab === cat.id ? (isDark ? "text-white" : "text-blue-600") : "text-zinc-500 hover:text-zinc-400"}`}>
              {activeTab === cat.id && <motion.div layoutId="activeTab" className={`absolute inset-0 rounded-xl shadow-sm ${isDark ? "bg-zinc-800" : "bg-white"}`} />}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {subModes[activeTab].map((sm) => (
            <button key={sm.id} onClick={() => setMode(sm.id)} className={`whitespace-nowrap px-6 py-2.5 rounded-full text-xs font-medium transition-all border ${mode === sm.id ? "bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-600/20" : (isDark ? "bg-zinc-900 border-zinc-800 text-zinc-400 hover:border-zinc-700" : "bg-white border-zinc-200 text-zinc-600 hover:border-zinc-300 shadow-sm")}`}>
              {sm.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="flex flex-col gap-3">
            ...
            <textarea value={input} onChange={(e) => setInput(e.target.value)} placeholder="Digite para traduzir..." className={`w-full h-64 p-6 rounded-[32px] text-lg outline-none transition-all resize-none border-2 ${isDark ? "bg-zinc-900/50 border-zinc-800 focus:border-blue-600/50 text-white" : "bg-white border-zinc-100 focus:border-blue-600/30 text-zinc-900 shadow-xl shadow-zinc-200/50"}`} />
          </div>
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center ml-2">
              <label className="text-xs font-bold uppercase tracking-widest text-zinc-500">Saída</label>
              <button onClick={() => { navigator.clipboard.writeText(output); alert("Copiado!"); }} className="text-[10px] font-bold bg-blue-600/10 text-blue-600 px-2 py-1 rounded hover:bg-blue-600/20 transition-colors">COPIAR</button>
            </div>
            <div className={`w-full h-64 p-6 rounded-[32px] text-lg overflow-y-auto break-words font-mono border-2 ${isDark ? "bg-zinc-900/30 border-zinc-800 text-blue-400" : "bg-blue-50/30 border-blue-100 text-blue-700 shadow-inner"}`}>
              <AnimatePresence mode="wait">
                <motion.span key={output} initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }} transition={{ duration: 0.2 }}>
        ...
        <footer className={`mt-12 p-8 rounded-[40px] border ${isDark ? "bg-zinc-900/50 border-zinc-800" : "bg-white border-zinc-100 shadow-sm"}`}>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <div><p className="text-2xl font-black text-blue-600">{input.length}</p><p className="text-[10px] font-bold uppercase text-zinc-500">Caracteres</p></div>
            <div><p className="text-2xl font-black text-blue-600">{input.split(/\s+/).filter(x => x).length}</p><p className="text-[10px] font-bold uppercase text-zinc-500">Palavras</p></div>
            <div className="col-span-2 text-left md:border-l md:border-zinc-800 md:pl-8 flex flex-col justify-center">
              <p className="text-xs text-zinc-500 leading-relaxed"><span className="font-bold text-blue-600">NATO Pro:</span> Agora suporta maiúsculas, minúsculas e pontuação!</p>
            </div>
          </div>
        </footer>
      </main>
      <p className="mt-8 text-[10px] font-bold text-zinc-600 uppercase tracking-[0.2em]">Engine v2.1 • NATO Enhanced</p>
    </div>
  );
}
